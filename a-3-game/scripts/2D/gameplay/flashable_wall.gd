...
func _physics_process(_delta: float) -> void:
	...
	# Check whether laser is currently being blocked. This is *not* calling the change_lit_status function because it should not be changing groups.w	if is_in_group("Disappeared") and collider and collider != laser_block.get_parent():
		var tween = create_tween()
		tween.tween_property(self, "modulate:a", 1.0, 0.3)
		await get_tree().create_timer(0.15).timeout
		set_collision_layer_value(1, true)
